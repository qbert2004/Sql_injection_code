...
Для каждого предсказания модели вычисляется **посимвольная атрибуция**: какой именно символ во входной строке вносит вклад в вердикт `INJECTION`.

```
Δᵢ = score(query) − score(query c i-м символом, заменённым на пробел)
```

| Δ | Интерпретация |
|---|----|
| **Δ > 0** | Символ **повышает** опасность (часть payload — `'`, `OR`, `--`, …) |
| **Δ < 0** | Символ **снижает** опасность (отвлекающая «маска») |
| **Δ ≈ 0** | Нейтральный символ |

### Артефакты

- `models/heatmap_01..10.png` — тепловая карта по 10 типичным запросам (boolean, UNION, time-based, comment, stacked, encoded + 4 безопасных)
- `heatmap_report.html` — интерактивный HTML с подсветкой каждого символа (hover → значение Δ)

### Запуск

```bash
# Сгенерировать отчёт по 10 встроенным примерам
...
### Чем отличается от SHAP / LIME

| Метод | Где применяется | Что показывает |
|------|----------------|----------------|
| **SHAP** (TreeExplainer) | Табличные модели (RF, XGB) | Вклад каждого числового признака |
| **LIME** | Любая модель | Локальный линейный суррогат |
| **Occlusion Heatmap** *(этот проект)* | Sequence-модели (CNN на символах) | **Позиция атакующего фрагмента в строке** |

Для оператора SOC это критично: вместо «признак №42 имеет вес 0.3» сразу видно, что подозрителен именно фрагмент `' UNION SELECT`.
